...
describe 'index file' do
  before(:all) do
    puppetserver.run_shell("#{CONFDIR}/common_events/collect_api_events.rb")
  end

  it 'index file exists' do
    expect(puppetserver.file_exists?("#{CONFDIR}/common_events/common_events_indexes.yaml")).to be true
  end

  it 'writes expected keys' do
    index_contents = puppetserver.run_shell("cat #{CONFDIR}/common_events/common_events_indexes.yaml").stdout
  ...
  it 'updates orchestrator index value' do
    ...
    index_contents = puppetserver.run_shell("#{CONFDIR}/common_events/collect_api_events.rb ; cat #{CONFDIR}/common_events/common_events_indexes.yaml").stdout
    index = YAML.safe_load(index_contents, [Symbol])
